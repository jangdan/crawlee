...
:::info Navigating automatic bot-protection avoidance
...
Let's explore the changes in more detail. We believe these modifications will enhance the readability and manageability of the crawler.
...
We won't go to great lengths here to talk about the `log` object from Crawlee, because you can read all about it in the <ApiLink to="core/class/Log">documentation</ApiLink>, but there's just one thing that we need to stress: **log levels**.
